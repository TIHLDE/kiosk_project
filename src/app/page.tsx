...
export default function Home() {
    ...
    return (
        <div
            className="bg-gradient-to-bl from-gray-100 to-gray-300 w-full h-screen text-white grid grid-cols-2 grid-rows-2 gap-4 p-4 justify-center">
            <div
                className="bg-gray-100 flex-box p-12 min-w-60 min-h-30 text-black flex flex-col items-center justify-center rounded-xl">
                {data ? (
                    <div
                        className="w-40 h-40 flex justify-center items-center rounded-full border-2 border-sky-700 shadow-md">
            ...
            <div
                className="bg-gray-100 flex-box p-12 text-black flex flex-col justify-center rounded-xl space-y-4">
                ...
                {data ? (
                    <div className="grid grid-cols-3 gap-3">
                        {data.mostSoldProductsByItems.slice(0, 9).map((product, index) => (
                            <div className="space-y-1 border-2 border-sky-700 rounded-lg p-2" key={index}>
            ...
            <div
                className="bg-gray-100 flex-box p-12 text-black flex flex-col items-center justify-center rounded-xl space-y-4">
                <h1 className="text-2xl font-bold">
                ...
                {data ? (
                        <div className="grid grid-cols-3 gap-3">
                            {data.mostSoldProductsByRevenue.slice(0, 9).map((product, index) => (
                                <div className="space-y-1 border-2 border-sky-700 rounded-lg p-2" key={index}>
                                    <p className="text-center font-semibold text-xl">
                                        {product.amount},-
                                    </p>
            ...
            <div
                ...
                {data ? (
                    <div
                        className="w-40 h-40 flex justify-center items-center rounded-full border-2 border-sky-700 shadow-md">
                        <p className="text-3xl font-bold"> {data.numberOfEnergyDrinksSold}</p>
                    </div>
                ) : (
                    <div>
                        <p>Loading...</p>
